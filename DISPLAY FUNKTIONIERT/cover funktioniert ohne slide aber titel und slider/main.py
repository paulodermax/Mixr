...
# --- CONFIG ---
# ZURÜCK AUF SICHERE GESCHWINDIGKEIT
BAUD_RATE = 921600
IMG_W = 120
IMG_H = 120
...
class LCD_1inch28(framebuf.FrameBuffer):
    def __init__(self):
        self.width = 240
        self.height = 240
        self.cs = Pin(CS, Pin.OUT); self.rst = Pin(RST, Pin.OUT)
        self.cs(1)
        ...
        self.init_display()
        self.pwm = PWM(Pin(BL)); self.pwm.freq(5000); self.pwm.duty_u16(65535)
...
class GraphicsEngine:
        ...
        if self.uart.any():
            try:
                ...
                if b'IMG_START' in line: 
                    ...
                    self.title = line.decode('utf-8').replace('TIT:', '').strip()
                    updated = True
                elif b'ART:' in line: 
                    self.artist = line.decode('utf-8').replace('ART:', '').strip()
                    updated = True
            except: pass
        return updated
    ...
    def load_image_data(self):
        ...
        while bytes_read < IMG_SIZE:
            ...
            if self.uart.any():
                ...
                if chunk:
                    self.curr_img[bytes_read : bytes_read + len(chunk)] = chunk
    ...
    def update(self):
        ...
        self.lcd.spi.write(self.lcd.buffer[0 : 28800])
        self.lcd.cs(1)
        
        # Text unten
        self.lcd.setWindows(0, 180, 240, 240)
        self.lcd.cs(1); self.lcd.dc(1); self.lcd.cs(0)
        self.lcd.spi.write(self.lcd.buffer[86400 : ])
        self.lcd.cs(1)
...
if __name__=='__main__':
    # BAUD RATE WIEDER NORMAL
    uart = UART(0, baudrate=BAUD_RATE, tx=Pin(16), rx=Pin(17))
    lcd = LCD_1inch28() 
    engine = GraphicsEngine(lcd, uart)
    
    # WICHTIG: Einmal sofort zeichnen, damit man was sieht!
    engine.update()
    
    print("Engine Running (Static)...")
    
    while True:
        # Nur updaten wenn was Neues kam
        if engine.receive_data():
            engine.update()
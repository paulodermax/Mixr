...
# Deine Pin-Definitionen bleiben exakt gleich
I2C_SDA = 6
I2C_SDL = 7
DC = 8
CS = 9
...
# UART hinzufügen für die Kommunikation (Pi Zero TX an GP1)
...
class LCD_1inch28(framebuf.FrameBuffer):
    ...
    def init_display(self):
        # Dein gesamter Initialisierungsblock (EF, EB, 84, etc.)
        self.rst(1)
        time.sleep(0.01)
        self.rst(0)
        time.sleep(0.01)
    ...
    def show(self):
        self.write_cmd(0x2A)
        self.write_data(0x00); self.write_data(0x00); self.write_data(0x00); self.write_data(0xef)
        self.write_cmd(0x2B)
        self.write_data(0x00); self.write_data(0x00); self.write_data(0x00); self.write_data(0xEF)
        self.write_cmd(0x2C)
...
# --- AB HIER IST NUR DER VISUALISIERUNGSPART GEÄNDERT ---
if __name__=='__main__':
    ...
    # Start-Screen
    LCD.fill(LCD.black)
    LCD.text("SYSTEM BEREIT", 65, 100, LCD.green)
    LCD.text("Warte auf Pi...", 65, 120, LCD.white)
    LCD.show()
    
    while(True):
        if uart.any():
            # Header lesen: 'T' für Text, 'B' für Bild
            header = uart.read(1)
            
            if header == b'T':
                # Songtitel anzeigen
                title = uart.readline().decode('utf-8').strip()
                LCD.fill(LCD.black)
                LCD.text("Now Playing:", 30, 80, LCD.green)
                LCD.text(title, 30, 110, LCD.white)
                LCD.show()
            
            elif header == b'B':
                # Bilddaten (Cover Art) empfangen
                uart.readinto(LCD.buffer)
                LCD.show()
